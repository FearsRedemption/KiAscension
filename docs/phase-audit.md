...
| 6. Ki UI/stats/dev inspection | Implemented foundation | Compact scouter-style HUD plus read-only stats/dev panels exist, including smaller text, badges, grouped sections, wrapped/clipped long lines, active drain visibility, locked-spell reasons, and temporary feel-debug readouts. Final UI art, advanced combat UI, and editable/config-gated dev tools are planned. |
...
| 13. Aura visuals by form | Implemented foundation | `AscensionAuraProfiles` drives form colors, dust, light, intensity, electric arcs, an eight-frame animated aura draw layer, charge-up flaring, power-down collapse, and Kaio-Ken overlay. The sheets are temporary placeholders; shader-level/final-art aura polish remains planned. |
| 14. Hair transformation cleanup | Implemented foundation | Hair profiles are separated from stats, natural hair is saved/restored for base, and regenerated tModLoader hair sheets keep matching `_Alt` textures. Current hair art is placeholder and true animated hair overlays remain planned. |
