...
The mod is buildable and has playable systems for the core DBZ-style loop: training, ki, forms, Kaio-Ken, representative techniques, starter melee, starter training stations, form-gated flight, and one early enemy. Many later phases are intentionally foundations or partial gameplay rather than final content. Final art/audio/UI polish, full melee, full training tiers, and custom bosses are not complete.
...
| 6. Ki UI/stats/dev inspection | Implemented foundation | Compact scouter-style HUD plus read-only stats/dev panels exist, including a smaller ki-focused panel, badges, grouped sections, wrapped/clipped long lines, active drain visibility, locked-spell reasons, and temporary feel-debug readouts. Final UI art, advanced combat UI, and editable/config-gated dev tools are planned. |
...
| 11. Skill feel/terrain/sound/visual behavior | Partially implemented gameplay | Existing skills now have stronger identity through tuned costs/speeds, distinct barrage patterns, short-lance Masenko, disk spin/cut visuals, heavier orb/ultimate handling, beam width/range/charge differences, temporary sound profiles, and shake feedback. Advanced cooldown/fatigue and final per-technique art/audio remain planned. |
...
| 13. Aura visuals by form | Implemented foundation | `AscensionAuraProfiles` drives form colors, dust, light, intensity, electric arcs, an eight-frame animated aura draw layer, charge-up flaring, power-down collapse, Kaio-Ken overlay, and layered/pulsing placeholder aura passes. The aura/electric sheets were regenerated for stronger Terraria-scale readability, but shader-level/final-art aura polish remains planned. |
| 14. Hair transformation cleanup | Implemented foundation | Hair profiles are separated from stats, natural hair is saved/restored for base, and regenerated tModLoader hair sheets keep matching `_Alt` textures. Custom hair swapping is active again with improved original placeholder silhouettes and face openings. Final hair art and true animated hair overlays remain planned. |
...
| 16. Items/accessories/crafting/training furniture | Partially implemented gameplay | Starter combat item, training items, starter placeable stations, `Ki Fragment`, and item framework docs exist. Current icons/tiles were regenerated as original Terraria-scale placeholders instead of vanilla/default stand-ins. Full accessory/armor/furniture set is planned. |
...
| 19. Documentation update | Implemented | Docs distinguish implemented gameplay, partial foundations, placeholders, and planned systems. This should be revisited after each gameplay pass. |
...
- Continue UI playtesting at multiple resolutions/UI scales before calling the HUD final.
