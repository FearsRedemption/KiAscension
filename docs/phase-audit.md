...
| 13. Aura visuals by form | Implemented foundation | `AscensionAuraProfiles` drives form colors, dust, light, intensity, electric arcs, an eight-frame animated aura draw layer, charge-up flaring, power-down collapse, Kaio-Ken overlay, and layered/pulsing placeholder aura passes. The sheets are temporary placeholders; shader-level/final-art aura polish remains planned. |
| 14. Hair transformation cleanup | Implemented foundation | Hair profiles are separated from stats, natural hair is saved/restored for base, and regenerated tModLoader hair sheets keep matching `_Alt` textures. Active gameplay currently uses natural hair style plus form tint/aura instead of swapping to rough placeholder hair sheets. Final hair art and true animated hair overlays remain planned. |
